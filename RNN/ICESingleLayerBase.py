...
class ICESingleLayer:
    def __init__(self, rnnModel, xlim, polyhedron_max_dim, use_relu=True, use_counter_example=False,
        ...
        # Hacky extensions to support ICE learning
        self.ice_alphas = [None] * self.dim * 2
        self.ice_betas = [None] * self.dim * 2
    ...
    def update_all_equations(self):
        ...
        for i, alpha in enumerate(self.alphas):
            ...
            for a in alpha:
                #print(a, i, self.rnn_start_idxs[i], self.rnn_output_idxs_double[i])
    ...
    def lin_template_to_marabou(self, isupper, alpha, beta, loop_idx, rnn_out_idx):
        inv_type = MarabouCore.Equation.LE if isupper else MarabouCore.Equation.GE
    ...
    def get_ice_equations(self):
        ...
        for i in range(len(self.ice_alphas)):
            self.equations.append([])
            eq = self.lin_template_to_marabou(
                self.ice_is_upper[i],
                self.ice_alphas[i],
                self.ice_betas[i],
                self.rnn_start_idxs[i], 
                self.rnn_output_idxs_double[i]
            )
            self.equations[-1].append(eq)
        return self.equations
