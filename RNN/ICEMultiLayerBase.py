...
from RNN.ICESingleLayerBase import ICESingleLayer
...
class ICEMultiLayer:
    ...
    def _initialize_single_layer(self, layer_idx: int) -> ICESingleLayer:
        if layer_idx == 0:
            prev_layer_lim = self.input_lim
        else:
            prev_layer_lim = None

        return ICESingleLayer(self.rnnModel, prev_layer_lim, polyhedron_max_dim=self.polyhedron_max_dim,
    ...
    def proved_ice_invariant(self, layer_idx=0, **kwargs):
        '''
        Call this function after proving invariant on layer_idx
        here input bounds for layer_idx+1
        :return: None
        ...
        orig_alphas = self.alphas_algorithm_per_layer[layer_idx].ice_alphas
        orig_betas = self.alphas_algorithm_per_layer[layer_idx].ice_betas
        l_bound = zip(orig_alphas[:len(orig_alphas)//2],
                      orig_betas[:len(orig_betas)//2])
        u_bound = zip(orig_alphas[len(orig_alphas)//2:],
                      orig_betas[len(orig_betas)//2:])
        l_bound = [[i] for i in l_bound]
        u_bound = [[i] for i in u_bound]
        #l_bound, u_bound = self.alphas_algorithm_per_layer[layer_idx].get_bounds()
        alphas_l, alphas_u, betas_l, betas_u = [], [], [], []
    ...
    def get_ice_equations(self, layer_idx=0):
        return self.alphas_algorithm_per_layer[layer_idx].get_ice_equations()
