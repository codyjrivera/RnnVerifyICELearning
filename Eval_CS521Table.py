...
@pytest.mark.parametrize(['net_path', 'n', 'point_idx'], product(*[multi_layer_paths[1:2], [2, 4, 6, 8, 10, 12, 14, 16], range(len(points))]))
def test_using_multilayer_gurobi(net_path, n, point_idx):
    ...
    idx_max, other_idx = Adversarial.get_out_idx(point, n, net_path, method)
    try:
        res, queries_stats, alpha_history = Adversarial.adversarial_query(point, 0.01, idx_max, other_idx, net_path,
                                                            gurobi_ptr, n)
        end = timer()
